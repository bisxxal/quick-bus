...
const busDetails = await getBooking(Number(id)); 
  return (
    <div className=' w-full h-full flex items-center text-gray-500 pt-10 justify-center'>
       <div className='buttonbg  max-md:w-[98%] w-5/6 bg-indigo-50 h-fit  pb-8 !rounded-xl overflow-hidden'>
       <p className='bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 mb-5 py-3 flex justify-between text-white px-3 bgbase w-full text-2xl max-md:text-lg font-bold '>{busDetails?.bus?.busName} <span>(Bus No.) {busDetails?.bus?.busNumber}</span></p>
       ...
       <div className='max-md:text-xs border-y-2 mt-5 border-dashed border-[#ffffff9a] flex justify-between items-center'>
         ...
         <div>
           <p className=' max-md:text-xl text-3xl '>Total price</p>
           <p className='  max-md:text-2xl text-4xl font-bold text-[#ffffff] '>₹ {busDetails?.bus?.price}</p>
       ...
       <div className='mt-5  max-md:text-sm max-md:flex-col max-md:gap-3 flex justify-between items-center'>    
