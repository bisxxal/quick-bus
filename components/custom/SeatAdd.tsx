...
const SeatAdd = ({busId , seats}:{busId:number,seats?:any}) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="border-2 p-4 max-md:text-xs  rounded-3xl bg-[#3352cc23]  border-[#3352CC] mt-5 w-full mx-auto ">

<h1 className=' mb-5 text-center  text-lg'>Add seats </h1>
    <div className=' flex    flex-wrap gap-3'>
    { seats && seats?.map((seat:any, index:number) => ( 
          <p key={index}
            className="bg-[#00ff007c] text-sm max-md:text-xs  border rounded-lg flex items-center justify-center border-[#00ff00] h-10 w-10"
              >{seat.seatNumber}
          </p> 
      ))}
    </div>
     ...
     {addSeat.map((seat, index) => (
        <div key={index} className=" text-xs max-md:text-xs  inline-block gap-2">
          <input
            required
...
<button
        ...
        className="border-2  max-md:text-base  rounded-2xl text-sm  border-[#3352CC] p-3"
      >
        Add Seat
      </button>
      </div>

      <button disabled={!busId} type="submit" className=" buttonbg rounded-2xl w-40  max-md:w-32 p-3">Submit</button>
