...
const TicketBar = ({fetchDir}:any) => {
  ...
  return (
    <form onSubmit={handelclicked} className=' w-[60%] max-md:w-[80%]  gap-2  backdrop-blur-lg rounded-[50px] max-md:p-4  max-md:rounded-[40px] h-32 max-md:h-80  flex items-center justify-center z-[20] mt-40  '>
     ...
     <select required className='outline-none black text-xl max-md:text-base font-semibold p-3 bg-transparent ' name="from"  >
        {
      ...
      <input id='myDate' required defaultValue={Date.now()} placeholder='date' className='max-md:w-full outline-none w-52 text-xl max-md:text-base font-semibold bg-transparent' type="date" name="date" />

      <button className='buttonbg max-md:mx-auto max-md:w-full text-xl max-md:text-base !font-bold p-6 py-5 w-44 max-md:py-3 ml-9' type="submit">Search</button>
     </div>
